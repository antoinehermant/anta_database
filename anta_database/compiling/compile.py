...
class CompileDatabase:
    def __init__(
        ...
        var_attrs_json: Optional[str] = None,
    ) -> None:
    ...
    def compile(self, cpus: int = cpu_count() - 1) -> None:
        ...
        if self._extract_data is True:
            ...
            print(
                "\n",
                "Will start extracting data from",
                num_tasks,
                "raw files\n" "\n   ",
                num_workers,
            ...
            if num_workers > 1:
                with Pool(num_workers) as pool:
                    for _ in tqdm(
                        pool.imap_unordered(self.extract, all_files_list),
                        total=num_tasks,
                        desc="Processing",
                        unit="file",
                    ):
        ...
        if self._hdf5 is True or self._netcdf is True:
            ...
            print(
                "\n",
                "Will start creating the hdf5/netcdf for",
                len(all_dirs),
                "traces\n" "\n   ",
                num_workers,
                "worker(s) allocated out of",
            ...
            if num_workers > 1:
                with Pool(num_workers) as pool:
                    for _ in tqdm(
                        pool.imap_unordered(self.combine_dfs, all_dirs), total=num_tasks
                    ):
                        pass
            else:
        ...
        if self._break_transects:
            ...
            print(
                "\n",
                "Breaking possible flight transects in separated h5 files for",
                num_tasks,
                "h5 files\n" "\n   ",
                num_workers,
                "worker(s) allocated out of",
            ...
            if num_workers > 1:
                with Pool(num_workers) as pool:
                    for _ in tqdm(
                        pool.imap_unordered(self.do_break_transects, all_h5),
                        total=num_tasks,
                    ):
                        pass
            else:
        ...
        if self._compute_distance:
            ...
            print(
                "\n",
                "Computing distances along transects for",
                num_tasks,
                "h5 files\n" "\n   ",
                num_workers,
                "worker(s) allocated out of",
            ...
            if num_workers > 1:
                with Pool(num_workers) as pool:
                    for _ in tqdm(
                        pool.imap_unordered(self.compute_distances, all_h5),
                        total=num_tasks,
                    ):
                        pass
            else:
        ...
        if self._compute_IRH_density:
            ...
            if num_workers > 1:
                with Pool(num_workers) as pool:
                    for _ in tqdm(
                        pool.imap_unordered(self.compute_irh_density, all_h5),
                        total=num_tasks,
                    ):
                        pass
            else:
        ...
        if self._compute_IMBIE_basins:
            ...
            if num_workers > 1:
                with Pool(num_workers) as pool:
                    for _ in tqdm(
                        pool.imap_unordered(self.compute_imbie_basins, all_h5),
                        total=num_tasks,
                    ):
                        pass
            else:
        ...
        if self._set_attributes:
            ...
            print(
                "\n",
                "Settings variable attributes in",
                num_tasks,
                "h5 files\n" "\n   ",
                num_workers,
                "worker(s) allocated out of",
            ...
            if num_workers > 1:
                with Pool(num_workers) as pool:
                    for _ in tqdm(
                        pool.imap_unordered(self.set_attrs, all_h5), total=num_tasks
                    ):
                        pass
            else:
                for h5_file in tqdm(all_h5, desc="Processing"):
            ...
            if self._remove_tmp_files:
                ...
                if num_workers > 1:
                    with Pool(num_workers) as pool:
                        for _ in tqdm(
                            pool.imap_unordered(self._cleanup, self._dir_list),
                            total=num_tasks,
                            unit="directory",
                        ):
                            pass
                    ...
                    for ds_dir in tqdm(
                        self._dir_list, desc="Removing", unit="directory"
                    ):
        ...
        if self._shapefiles:
            ...
            print(
                "\n",
                "Will start creating the shapefiles for",
                len(self._dir_list),
                "datasets\n" "\n   ",
                num_workers,
                "worker(s) allocated out of",
            ...
            if num_workers > 1:
                with Pool(num_workers) as pool:
                    for _ in tqdm(
                        pool.imap_unordered(self.make_shapefile, self._dir_list),
                        total=num_tasks,
                    ):
                        pass
            else:
        ...
        if self._geopackages:
            ...
            print(
                "\n",
                "Will start creating the geopackages for",
                len(self._dir_list),
                "datasets\n" "\n   ",
                num_workers,
                "worker(s) allocated out of",
            ...
            if num_workers > 1:
                with Pool(num_workers) as pool:
                    for _ in tqdm(
                        pool.imap_unordered(self.make_geopackage, self._dir_list),
                        total=num_tasks,
                    ):
                        pass
            else:
    ...
    def extract(self, file_dict) -> None:
        ...
        original_new_columns = pd.read_csv(
            f"{file_dict['dir_path']}/original_new_column_names.csv"
        )
        ...
        ds.columns = (
            original_new_columns[ds.columns].iloc[0].values
        ...
        if "PSX" in ds.columns and "PSY" in ds.columns:
            ...
            mask = ds["PSX"].isin(values_to_replace) & ds["PSY"].isin(values_to_replace)
            if mask.any():
        ...
        if (
            "ICE_THK" in ds.columns
            and "SURF_ELEV" in ds.columns
            and not "BED_ELEV" in ds.columns
        ):
            ds["BED_ELEV"] = ds["SURF_ELEV"] - ds["ICE_THK"]
        if (
            "ICE_THK" in ds.columns
            and "BED_ELEV" in ds.columns
            and not "SURF_ELEV" in ds.columns
        ):
            ds["SURF_ELEV"] = ds["BED_ELEV"] + ds["ICE_THK"]
        if (
            "SURF_ELEV" in ds.columns
            and "BED_ELEV" in ds.columns
            and not "ICE_THK" in ds.columns
        ):
        ...
        if "PSX" not in ds.columns and "PSY" not in ds.columns:
            if "lon" in ds.columns and "lat" in ds.columns:
                ...
                ds["PSX"], ds["PSY"] = transformer.transform(
        ...
        if "raw file" in raw_md.columns:
            ...
            ds["Flight_ID"] = ds["Flight_ID"].str.replace(
                r"/\s+", "_"
            )  # Replace slashes with underscores, otherwise the paths can get messy
            ...
            for flight_id in flight_ids:
                ...
                if "acq_year" in pattern_values.columns:
                    ...
                    if not pd.isna(pattern):
                        ds_trace["acq_year"] = ds_trace["acq_year"].apply(
                            lambda x: extract_year(x, pattern)
                        )
                        unique_time = np.unique(ds_trace["acq_year"])
                        if len(unique_time) > 1:
                            raise ValueError(
                                f"flight {flight_id} in {file_name_} contains {len(unique_time)} different acquisition year. Current code does not support this"
                            )
                        else:
                            position = pattern.find("YYYY")
                ...
                if age is not pd.NA:
                    ...
                    if (
                        not isinstance(institute, (list, tuple, set))
                        and institute != "nan"
                        and dataset in ["BEDMAP2", "BEDMAP3"]
                    ):
            ...
            for IRH, row in IRH_md.iterrows():
                if IRH in ds.columns:
                    ...
                    if (
                        ...
                        ds_trace_file = (
                            f"{file_dict['dir_path']}/pkl/{flight_id}/{IRH}.pkl"
                        )
                    flight_dir, _ = os.path.split(ds_trace_file)
                    os.makedirs(flight_dir, exist_ok=True)
    ...
    def combine_dfs(self, trace_dir: str) -> None:
        ...
        if len(files) > 1:
            ...
            for col in self._var_list:
                matching_cols = [
                    c for c in merged_df.columns if col.lower() in c.lower()
                ]
                if matching_cols:
                    temp_col = f"temp_{col}"
                    merged_df[temp_col] = (
                        merged_df[matching_cols].bfill(axis=1).iloc[:, 0]
                    )
        ...
        if ages:
            ...
            if "IRH_DEPTH" in ds.variables:
                ds["IRH_DEPTH"] = ds["IRH_DEPTH"].chunk(
                    {"IRH_AGE": 1, "point": chunk_size_point}
                )
                encoding["IRH_DEPTH"] = {
                    "zlib": True,
                    "complevel": 1,
                    "chunksizes": (chunk_size_point, 1),
                }
            ...
            with xr.open_dataset(f, engine="h5netcdf") as ds:
                ...
                if flight_id_flag == "not_provided":
                    ...
                    for t in range(len(break_points) - 1):
                        ds_flight_line = ds.isel(
                            point=slice(break_points[t], break_points[t + 1])
                        ).copy()
                        ...
                        ds_flight_line.to_netcdf(
                            h5_flight_file, engine="h5netcdf", mode="a"
                        )
            ...
            with xr.open_dataset(f, engine="h5netcdf") as ds:
                ...
                if "ICE_THK" in ds.variables and "IRH_DEPTH" in ds.variables:
                    ...
                    if point_shape < 1e6:
                        ...
                        chunk_size_point = 10000
                    ds["IRH_FRAC_DEPTH"] = ds["IRH_DEPTH"].chunk(
                        {"IRH_AGE": 1, "point": chunk_size_point}
                    )
                    encoding = {
                        "IRH_FRAC_DEPTH": {
                    ...
                    ds.to_netcdf(
                        h5_file, engine="h5netcdf", mode="a", encoding=encoding
                    )
        ...
        with h5py.File(h5_file, "a") as f:
            with xr.open_dataset(f, engine="h5netcdf") as ds:
                ...
                while remaining_indices:
                    ...
                    nearest_idx = min(
                        remaining_indices, key=lambda idx: distances[last_idx, idx]
                    )
        ...
        with h5py.File(h5_file, "a") as f:
            with xr.open_dataset(f, engine="h5netcdf") as ds:
                ...
                points = gpd.GeoDataFrame(
                    coords, geometry=geometry, crs=self._basins.crs
                )
                joined = gpd.sjoin(
                    points, self._basins, how="inner", predicate="within"
                )
                lookup_df = joined[joined["Regions"] != "Islands"][
                    ["Subregion", "Regions"]
                ].drop_duplicates()
    ...
    def make_shapefile(self, dataset_dir: str) -> None:
                    ...
                    df = gpd.GeoDataFrame(
                        {
                            "geometry": [
                                Point(x, y)
                                for x, y in zip(ds.PSX.values, ds.PSY.values)
                            ],
        ...
        df_points = gpd.GeoDataFrame(
            pd.concat(all_dfs, ignore_index=True), crs="EPSG:3031"
        )
    ...
    def make_geopackage(self, dataset_dir: str) -> None:
        ...
        df_points = gpd.GeoDataFrame(
            pd.concat(all_points, ignore_index=True), crs="EPSG:3031"
        )
