...
from typing import Union, Dict, TYPE_CHECKING, Optional, List
...
class Plotting:
    ...
    def _get_coordinates_for_plotting(
        self, file_paths: List[str], downsampling_factor: Optional[int] = None
    ) -> pd.DataFrame:
        """
        Get coordinate data (PSX, PSY) for plotting.

        For CloudDatabase, this uses the zarr data generator.
        For regular Database, this uses direct HDF5 file access.
        """
        if self._is_cloud_database():
            ...
            # For regular Database, use direct HDF5 access
            all_x, all_y = [], []
            for f in file_paths:
    ...
    def _open_dataset_for_plotting(self, file_path: str):
        """
        Open a dataset for plotting, using the appropriate method for the database type.

        For CloudDatabase: opens zarr dataset from S3
        For regular Database: opens HDF5 dataset locally
        """
        if self._is_cloud_database():
            # For CloudDatabase, convert to S3 zarr path and open
            s3_zarr_path = self._db._get_s3_zarr_path(file_path)
            try:
                import xarray as xr

                # First try the same approach that works directly
                try:
                    ...
                    try:
                        ...
                        if self._db.aws_credentials:
                            storage_options = {
                                "key": self._db.aws_credentials["key"],
                                "secret": self._db.aws_credentials["secret"],
                            }
                        else:
                            storage_options = {"anon": True}

                        # Create S3 filesystem and mapper
            ...
            # For regular Database, open HDF5 file
            full_path = os.path.join(self._db._db_dir, file_path)
            import xarray as xr
            ...
            try:
                ...
                print(f"Failed to open HDF5 dataset {full_path}: {e}")
                return None
    ...
    def _base_plot(
        ...
        if color_by == "dataset":
            ...
            remaining_dataset = [
                dataset for dataset in datasets if dataset not in bedmap_entries
            ]
            color_indices = np.linspace(0.1, 0.9, len(remaining_dataset))
            if cmap is None:
                cmap = self._custom_cmap()
            colors = {
                dataset: cmap(i) for i, dataset in zip(color_indices, remaining_dataset)
            }
            ...
            for dataset in tqdm(
                ...
                metadata_impl = self._db.query(
                    dataset=dataset, flight_id=flight_ids, retain_query=False
                )
        ...
        if color_by == "institute":
            ...
            for institute in tqdm(
                ...
                metadata_impl = self._db.query(
                    institute=institute, flight_id=flight_ids, retain_query=False
                )
                file_paths = self._db._get_file_paths_from_metadata(metadata_impl)
                file_paths = np.unique(file_paths)
        ...
        if color_by == "var":
            ...
            if var == "IRH_NUM":
                ...
                if len(age) > 1:
                    print(
                        "WARNING: Multiple layers provided: ",
                        age,
        ...
        if color_by == "transect_1D":
            ...
            ds = self._open_dataset_for_plotting(f)
            if ds is None:
                print(f"Failed to open dataset for transect plotting")
                return
            ...
            for flight_id in tqdm(
                ...
                file_paths = self._db._get_file_paths_from_metadata(metadata_impl)
                file_paths = np.unique(file_paths)
                # Use the unified coordinate extraction method (no downsampling for flight_id plotting)
        ...
        if color_by in ["dataset", "institute"]:
            ...
            plt.gcf().set_size_inches(
                10 * scale_factor * ncol / 1.15, 10 * aspect_ratio * scale_factor
            )
