...
class Database:
    def __init__(
        self,
        database_dir: str,
        file_db: str = "AntADatabase.db",
        index_database: bool = False,
        include_BEDMAP: bool = False,
        max_displayed_flight_ids: Optional[int] = 50,
    ) -> None:
            ...
            if isinstance(radar_instrument, list):
                ...
                for radar in radar_instrument:
                    like_conditions.append(
                        "radar_instruments_list.radar_instruments LIKE ?"
                    )
        ...
        if self._excluded.get("radar_instrument"):
            if isinstance(self._excluded["radar_instrument"], list):
                ...
                for ri in self._excluded["radar_instrument"]:
                    not_like_conditions.append(
                        "radar_instruments_list.radar_instruments NOT LIKE ?"
                    )
                    params.append(f"%{ri}%")
                conditions.append("(" + " AND ".join(not_like_conditions) + ")")
            else:
    ...
    def query(
        ...
        metadata = {
            ...
            "_filter_params": {
                "dataset": self._excluded["dataset"],
                "institute": self._excluded["institute"],
                "project": self._excluded["project"],
                "acquisition_year": self._excluded["acquisition_year"],
                "age": self._excluded["age"],
                "var": self._excluded["var"],
                "flight_id": self._excluded["flight_id"],
                "region": self._excluded["region"],
                "IMBIE_basin": self._excluded["IMBIE_basin"],
                "radar_instrument": self._excluded["radar_instrument"],
            },
            "database_path": self._db_dir,
            "file_db": self._file_db,
        }
        ...
        metadata["DOI_publication"] = list(
            dict.fromkeys(df_dataset_sorted["DOI_publication"])
        )
    ...
    def data_generator(
        ...
        file_paths = self._get_file_paths_from_metadata(metadata=md)
        file_paths = np.unique(
            file_paths
        )  # Be carefull as many pointers point to the same file
        ...
        for file_path in tqdm(
            ...
            df = pd.DataFrame(
                {
                    "PSX": ds["PSX"][::downsampling_factor],
    ...
    def __repr__(self):
        ...
        if len(flight_ids) > self._max_displayed_flight_ids:
            ...
            flight_id_str = (
                ...
                + f", [ ... ] , "
                + ", ".join(last_20)
                + f" (found {len(flight_ids)}, displayed {self._max_displayed_flight_ids})"
            )
        else:
